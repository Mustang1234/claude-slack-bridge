...
def api(token: str, method: str, payload: dict, form: bool = False) -> dict:
    ...
    except (TimeoutError, OSError, http.client.HTTPException, json.JSONDecodeError) as e:
        # SSL 읽기 타임아웃은 URLError 로 안 싸여 그대로 튀어나온다. 이걸 안
        # 잡으면 지연 한 번에 감시자가 통째로 죽는다 — 실측으로 그렇게 죽었다.
        # 여기서 SlackError 로 바꿔 놓아야 위쪽의 재시도가 받아낸다.
        #
        # 응답 본문을 받는 도중 연결이 끊기면 IncompleteRead 가 나는데, 이것은
        # OSError 가 아니라 http.client.HTTPException 이다. Wi-Fi 가 불안정한 날
        # conversations.replies(본문 100KB 안팎)가 이걸로 지킴이를 열다섯 번 넘게
        # 죽였다(keeper.log 실측, 2026-09-15~17). 서버가 30초 안에 되살리긴 하지만
        # 그 사이 수신확인 유예 목록이 날아가고, 서버까지 없으면 아무도 안 살린다.
