...
# 세션이 말을 못 받고 있는 것을 폰에 드러내는 기준. 침묵은 "일하는 중" 과
# "죽었음" 이 똑같이 보이는 유일한 상태라, 그 구분을 사람에게 맡기지 않는다.
SILENCE_LEAD = 5 * 60     # 사용자의 말이 이만큼 답 없이 남으면 막힌 것으로 본다
DOWN_GRACE = 60.0         # 이 상태가 이만큼 이어져야 알린다 — 재기동 순간의 오탐 방지
...
def down_text(reason: str) -> str:
    """세션이 말을 못 받고 있다고 폰에 알리는 한 줄.

    받는 사람이 알아야 하는 것은 둘이다 — 지금 내 말이 처리되지 않는다는 것,
    그리고 그 말이 사라지지는 않는다는 것. 뒤엣것을 빼면 사용자가 같은 말을
    다시 쓰게 된다.
    """
    return (
        f"⚠️ *세션이 응답하지 않습니다* — {reason}\n"
        "지킴이는 살아 있어 답장은 이 스레드에 남습니다. "
        "세션이 돌아오면 이어서 처리하고, 돌아온 것도 여기에 알립니다."
    )


def back_text(down_seconds: float) -> str:
    """돌아왔다고 알리는 한 줄.

    경고만 보내고 조용해지면, 복구된 것과 여전히 죽어 있는 것이 폰에서
    구분되지 않는다. 경고를 보냈으면 그 끝도 반드시 보낸다.
    """
    return f"✅ *세션이 돌아왔습니다* — {fmt_remaining(down_seconds)} 만에 다시 듣고 있습니다."
