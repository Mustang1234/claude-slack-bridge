...
툴 사용법은 서버가 함께 배포하므로(MCP `instructions`) 따로 적을 것이 없다. 남는 것은
**언제 부를지** 하나이고, 그건 각자의 `~/.claude/CLAUDE.md` 에 한 줄로 적는다.

```markdown
자리를 비운 사이 알아야 할 일(조사 완료·장시간 작업 종료·결정이 필요한 갈림길)이
생기면 기다리지 말고 `slack_notify` 로 먼저 보낸다. "폰으로 얘기하자" 고 하면
`slack_chat_open` 으로 스레드를 연다.
```

이 줄이 없으면 툴은 붙어 있어도 잘 불리지 않는다. 알림은 사람이 요청해서 나가는 것이
아니라 Claude 가 먼저 보내야 하는 것이라, 그 판단 기준을 주지 않으면 그냥 조용하다.
...
답하는 순간 꺼진다. 메시지가 아니라 알림이 가지 않고 이력에도 남지 않는다.
스레드를 닫으면 세션도 닫힌다.
